...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const gangNavLinks = document.querySelectorAll(".gang-file-nav a[href^='#']");
    const gangSections = document.querySelectorAll(".gang-section");

    if (gangNavLinks.length && gangSections.length) {
    ...
    const gangToggle = document.getElementById("gang-music-toggle");
const gangText = document.querySelector(".gang-music-display strong");
const gangAudio = document.getElementById("gang-music");

if (gangToggle && gangText && gangAudio) {
    const originalThemeName = gangText.textContent
        .replace(" : PAUSED", "")
        .replace(" : PLAYING", "");
    ...
    function setDisplay() {
        if (gangAudio.paused) {
            gangText.textContent = `${originalThemeName} : PAUSED`;
            gangToggle.textContent = "▶";
        } else {
            gangText.textContent = `${originalThemeName} : PLAYING`;
            gangToggle.textContent = "❚❚";
        }
    }
    ...
    gangToggle.addEventListener("click", (e) => {
        e.preventDefault();
        e.stopPropagation();

        if (gangAudio.paused) {
            gangAudio.play().then(setDisplay).catch(setDisplay);
        } else {
            gangAudio.pause();
            setDisplay();
        }
